...
Download the city's OSM PBF and the official GTFS ZIP files for every transit agency you need, then place them together without unpacking the GTFS archives:

```sh
mkdir -p "$HOME/housing-network"
cp city.osm.pbf agency-one.zip agency-two.zip "$HOME/housing-network/"
```
