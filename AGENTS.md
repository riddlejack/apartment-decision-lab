...
- Refresh: inspect configured sources, run `housing collect` / `housing refresh`, report source statuses and unresolved facts. For a useful rendered-page gap, follow [browser-assisted collection](docs/BROWSER-COLLECTION.md) and resume `housing assisted plan`; verify website filters because saved criteria are not applied automatically.
