...
Use the CLI before inventing scraping or routing workflows. Start with `goldblum doctor` and `goldblum status --json` (or `uv run housing ...` in a checkout). `--workspace PATH` goes before the subcommand. Read only the affected adapter/exception; do not load a database or full HTML archive into context.
